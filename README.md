...
This function also saves the manager name and ID of the user data as a CSV mapping file (**entry_to_player_mapping.csv**). 

### Get user data across time

Using the ```league_{league_id}_entry_to_player_mapping.csv``` mapping file, we can get the user data across the course of the league. In other words, we can get the number of points for each gameweek. 
...
## Fixture Difficulty Rating (FDR) Representational Dissimilarity Matrices (RDM)
