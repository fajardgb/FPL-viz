"""
charts.py — matplotlib chart rendering. Every function takes already-computed
data plus a save_dir and writes a PNG; no fetching or analysis happens here.
"""
import os
...
matplotlib.use("Agg")  # headless rendering on a server
...
def plot_cover_page(league_name: str, gw: int, save_dir: str) -> None:
    plt.figure(figsize=(11, 8.5))
    plt.axis("off")
    plt.text(0.5, 0.6, league_name, ha="center", va="center", fontsize=32, fontweight="bold")
    plt.text(0.5, 0.48, f"Gameweek {gw} Report", ha="center", va="center", fontsize=20, color="dimgray")
    plt.tight_layout()
    _save(f"{save_dir}/cover.png")
    plt.close()
...
def plot_captaincy(team_df: pd.DataFrame, gw: int, save_dir: Optional[str] = None) -> None:
    sorted_df = team_df[team_df["captaincy_count"] > 0].sort_values("captaincy_count", ascending=False)
...
def plot_most_owned(team_df: pd.DataFrame, gw: int, n: int = 20, save_dir: Optional[str] = None) -> None:
...
def plot_differentials_by_price(
    team_df: pd.DataFrame, gw: int, own_max: int, top_n: int = 10, save_dir: Optional[str] = None
) -> None:
    diff_df = team_df[team_df["own_count"] <= own_max].sort_values("price", ascending=False).head(top_n)
...
def plot_differentials_by_points(
    ...
    diff_df = team_df[team_df["own_count"] <= own_max].sort_values("points", ascending=False).head(top_n)
...
def plot_transfers(df_in: pd.DataFrame, df_out: pd.DataFrame, gw: int, save_dir: Optional[str] = None) -> None:
    plt.figure(figsize=(10, 6))
...
def plot_top3_current_standings(standings_df: pd.DataFrame, save_dir: Optional[str] = None) -> None:
    ...
    if {"rank", "player_name", "total_points"}.issubset(standings_df.columns):
        ...
        plot_df = points_series.sort_values(ascending=False).rename_axis("player_name").reset_index(name="total_points")
    ...
    bars = plt.barh(plot_df["player_name"], plot_df["total_points"], color=colors, edgecolor="black", linewidth=1.5)
...
def plot_top3_rank_history(rank_history_df: pd.DataFrame, top3_names: list[str], save_dir: Optional[str] = None) -> None:
...
def plot_top3_stats(rank_stats_df: pd.DataFrame, save_dir: Optional[str] = None) -> None:
...
def plot_top3_detailed_stats(top3_names: list[str], detailed_stats: dict, save_dir: Optional[str] = None) -> None:
    ...
    for idx, (ax, manager_name) in enumerate(zip(axes, top3_names)):
        ...
        chips_summary = ", ".join(chip_names.get(chip, chip) for chip, remaining in chips_left.items() if remaining)
